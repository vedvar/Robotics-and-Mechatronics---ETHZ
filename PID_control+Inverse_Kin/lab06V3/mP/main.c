...
int main()
{
  ...
  if ((task_selection == 4) || (task_selection == 5) || (task_selection == 6))
  {
    ...
    double k_p = 0.09; //underdamped: k_p=0.09 || critically: k_p=0.06 || overdamped: k_p:0.04
    double k_d = 0.03; //underdamped: k_d=0.03 || critically: k_d=0.03 || overdamped: k_d:0.04
    double k_i = 0.02; //underdamped: k_i=0.02 || criticalls: k_i=0.02 || overdamped: k_i:0.02
    ...
    */
    ...
    //get inital time to have the net time afterwards
    double initial_time = getMicroseconds();
    while (1)
    {
      ...
      if (flag)
      {
        ...
        case 5: //TODO: Postlab Q5 step response reference  --> use function in util.h 
          ...
          //get the current time but subtract the initial time from it!!
          stepResponse((getMicroseconds() - initial_time)/1000000.0, &x_ref, &y_ref, &vx_ref, &vy_ref);
          break;

        case 6: //TODO: Postlab Q6 circular trajectory reference --> implement & use function in util.h 

          //get the current time but subtract the initial time from it!!
          circularTrajectory((getMicroseconds() - initial_time)/1000000.0, &x_ref, &y_ref, &vx_ref, &vy_ref);
          break;
        }
        ...
        x_integ += dt*((e_x+prev_e_x)/2);         y_integ += dt*((e_y+prev_e_y)/2);
